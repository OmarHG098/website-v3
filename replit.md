...
-   **Content Management System (CMS)**: A YAML-based system allows marketing teams to manage content for career programs, landing pages, and location pages without code changes. Content is stored in:
    ...
    - `marketing-content/locations/{slug}.{locale}.yml` for location pages (e.g., `miami-usa.en.yml`, `miami-usa.es.yml`)
    All content types share the same structure with `meta`, `schema`, and `sections` properties, dynamically rendered by a unified `SectionRenderer` component.
